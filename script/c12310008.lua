...
function s.atkLimitFilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:GetBaseAttack()<1700
end
function s.actcon(e)
	local tp=e:GetHandlerPlayer()
	local a=Duel.GetAttacker()
	local d=Duel.GetAttackTarget()
	return (a and s.atkLimitFilter(a,tp) and Duel.GetTurnPlayer()==e:GetHandlerPlayer()) or (d and s.atkLimitFilter(d,tp) and Duel.GetTurnPlayer()==e:GetHandlerPlayer())
end
